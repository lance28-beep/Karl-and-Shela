      ...
      <div className="text-center mb-8">
        ...
        <div className="mt-4 bg-gradient-to-r from-teal/10 to-sand/20 rounded-xl p-4 sm:p-6 border border-teal/20 max-w-2xl mx-auto">
          <div className="flex items-center justify-center gap-3 mb-3">
            ...
            <h3 className="text-lg sm:text-xl font-playfair font-bold text-ink">
          ...
          {isLoading ? (
            <div className="flex items-center justify-center h-40">
              <div className="flex flex-col items-center gap-3">
                ...
                <span className="text-teal font-lora">Loading guests...</span>
              </div>
            </div>
          ) : error ? (
            ...
            <div className="flex items-center justify-center h-40 text-ink/60">
              <div className="flex flex-col items-center gap-3">
                <MessageSquare className="h-8 w-8" />
                <span className="font-lora">No guests have RSVP'd yet</span>
              </div>
            </div>
          ) : (
              ...
              {guests.map((guest, index) => (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                    <div>
                      <h4 className="font-lora text-ink text-lg flex items-center gap-2">
                        <Users className="h-4 w-4 text-teal" />
                        {guest.name}
                      </h4>
                      <div className="flex items-center text-sm text-ink/60 mt-1">
                        <Mail className="h-4 w-4 mr-1 text-teal" />
                        <span className="font-lora">{guest.email}</span>
                      </div>
                    </div>
                    <div className="bg-teal/20 text-teal border border-teal/30 font-lora px-3 py-1 rounded-full text-sm">
                  ...
                  {guest.message && (
                    <div className="mt-3 pt-3 border-t border-teal/20">
                      <div className="flex items-start">
                        <MessageSquare className="h-4 w-4 mr-2 mt-0.5 text-teal/60" />
                        <p className="text-sm text-ink/80 font-lora">{guest.message}</p>
                      </div>
                    </div>
                  )}

                  <div className="mt-3 text-xs text-ink/40 flex items-center gap-1.5">
